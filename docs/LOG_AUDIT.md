...
- Validated against a real local PostgreSQL 17 instance (2026-08-01): `python -m scripts.init_db` created the `agent_logs` schema, `python -m scripts.generate_test_logs` wrote all 6 simulated rows with zero `central_log_write_failed` entries, confirmed by querying `agent_logs` directly. The automated test suite still runs against an in-memory SQLite database (`tests/conftest.py`) for speed and CI independence from a live server.
